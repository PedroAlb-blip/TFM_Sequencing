...
def reader (path, fmt="abi"):
   ...
   if "-R_" in path: ### Here the indicator for reverse primer
      sequence = str(record.annotations["abif_raw"]["PBAS2"]).replace("b","").replace("'","")[::-1] ### This turns around the reverse channels
      sequence = sequence.replace("A","Z").replace("G","X").replace("T","A").replace("C","G").replace("Z","T").replace("X","C") ### This complements the reverse channels
      ...
      guide = str(record.annotations["abif_raw"]["FWO_1"]).replace("b","").replace("'", "").replace("A","Z").replace("G","X").replace("T","A").replace("C","G").replace("Z","T").replace("X","C") ### This complements the guide
...
def main(path_fw, path_rv):
   ...
   try:
      name=path_fw.split("\\")[-1].split("_")[2] + ".txt" ###This generates the name of the file by using the path
      file=open(f"Sequences\\{name}", "x")
   except IOError:
      file=open(f"Sequences\\{name}", "w")
   ### Then, here, all the information required is added, sequences, plocs and then channels and guides
...
### A neat trick is used to eliminate repeated file names using the dictionary properties that no two keys can be the same
file_purge = dict(enumerate(files))
egrup_elif = {v : k for k, v in file_purge.items()}
unique = list(egrup_elif.keys())
### The unique files are produced to ease finding the forward and reverse .ab1 files
for i in unique:
   fw, rv = "", ""
   for j in file_list:
      if i in j and "-F_" in j: ### Here the indicator for forward primer to later call main
         fw = str(path_folder + "\\" + j)
      elif i in j and "-R_" in j: ### Here the indicator for reverse primer to later call main
