...
def reader (path, fmt="abi"):
   ...
   channels = dict()
   if "_VP4RN_" in path: ### Here the indicator for reverse primer
   ...
   elif "_VP4FC_" in path: ### Here the indicator for forward primer
...
path_folder = "c:\\Users\\Pedro\\Downloads\\secuenciassolicitudcps20250627_sg07vp4"
...
for i in unique:
   for j in file_list:
      if i in j and "_VP4FC_" in j:
         fw = str(path_folder + "\\" + j)
      elif i in j and "_VP4RN_" in j:
