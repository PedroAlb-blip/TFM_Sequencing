...
In order for DCYFR to work on your computer various programs are needed, in this section I detail the programs and versions used to produce our results 
which, with due tinkering, should also work in any computer.

MAFFT: mafft-7.526-win64-signed.zip must be installed from the mafft official page https://mafft.cbrc.jp/alignment/software/windows_without_cygwin.html
and the folder resulting from installation must be added to the PATH, for instance in my case the MAFFT folder is added under C:\WINDOWS which in turn 
is added to the path, you can either add that same folder C:\WINDOWS to PATH or add some other folder to the PATH in which the file \MAFFT is present.

The other main thing is the python library keras, to avoid any mishappenings the version which I utilise in these scripts is 3.10.0. perhaps other keras 
versions might work but I would install 3.10.0 to prevent possible incompatibility. Also bear in mind that the folders and paths present within the python 
codes must be changed to accomodate to your especific system.
...
The DiNN-.py series of files are capable of taking the files produced by the parsing, selecting the peaks by peak discovery, and running an instance of the 
deep neural network to produce binary vectors ultimately encoding the sequence for forward and reverse strands, then the sequences are fully fledged out
and the respective sequences are aligned. If the file running is DINN8.py then the consensus sequence is produced by selection of the biggest possible 
chunk of alignment, going through the alignment and selecting a region in which at least 5 consecutive peaks in the reverse sequence have a better read
than those in the forward, if none is found then it automatically switches to the reverse once the chunk ends. DINNAMITE.py on the other hand produces 
the alignment, finds the gaps under a certain regime (for instance 5 matches followed by a mismatch or more and 3 more matches to ensure good envelopment),
and then it iterates over the channels corresponding to the alignment producing a weighted summation of channels corresponding to the position within the
alignment (i.e. regions closer to 5' will add a higher weight to the forward sequence as the reads are of better quality while near 3' the reverse will
have a higher weight). 

To produce a good addition the channels are resized, this is performed to ensure that peaks somewhat coincide between the channels, to do this a stitching 
method is used, the alignment region within the channels is divvied into as many segments as peaks are detected, picking the larger choice between forward 
and reverse, then the minima of channel summations are detected and removed by performing a geometric mean between the minimum and either the next or the 
previous point. Once the channels have been summed the regions within the original channels variables are swapped for the summed channels then the peak 
discovery followed by neural network adscription, sequence generation, and alignment are performed again and as many times as the correction factor warrants
finally once the correction loop is exited the DINNAMITE sequence is generated by mimicking the sequence attainment in DINN8.py.
